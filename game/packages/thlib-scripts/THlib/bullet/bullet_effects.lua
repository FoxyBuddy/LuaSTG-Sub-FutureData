...
function bullet_fog:init(master_bullet,color_index,time,max_scale,min_scale,use_bullet_img)
    --捕获父子弹对象
    self.master_bullet = master_bullet
    --弹雾持续时间
    self.stay_time = time
    --弹雾最大初始缩放
    self.max_scale = max_scale or 3
    --弹雾最小初始缩放
    ...
    self.use_bullet_img = use_bullet_img or false
    ...
    lerp_to(self,"vscale",4,self.stay_time,self.min_scale)
    lerp_to(self,"hscale",4,self.stay_time,self.min_scale)
